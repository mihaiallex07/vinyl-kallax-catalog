...
function SuggestField({ label, value, onChange, options, placeholder }: { label: string; value: string; onChange: (value: string) => void; options: string[]; placeholder?: string }) {
  ...
  const visibleOptions = options.filter((option) => !value || option.toLocaleLowerCase("ro").includes(value.toLocaleLowerCase("ro"))).slice(0, 40);
  const currentLabel = manual ? (value || "Introdu manual…") : (value || placeholder || "Selectează o valoare");
  const choose = (next: string) => { if (next === "__manual__") { setManual(true); onChange(""); } else { setManual(false); onChange(next); } setOpen(false); };
  return <div className="cream-select-field" onBlur={() => window.setTimeout(() => setOpen(false), 120)}><label>{label}<button type="button" className="cream-select-trigger" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)}><span>{currentLabel}</span><ChevronDown size={16} /></button></label>{open && <div className="cream-select-menu" role="listbox">{visibleOptions.map((option) => <button type="button" role="option" aria-selected={option === value} className={`cream-select-option${option === value ? " selected" : ""}`} key={option} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(option)}>{option}</button>)}<button type="button" role="option" className="cream-select-option manual-option" onMouseDown={(event) => event.preventDefault()} onClick={() => choose("__manual__")}>Introdu manual…</button></div>}{manual && <label className="manual-inline">Valoare nouă<input autoFocus value={value} onChange={(event) => onChange(event.target.value)} placeholder="Scrie valoarea nouă" /></label>}</div>;
}
